...
La instalación de dependencias requiere `venv` y `ensurepip`. Algunas distribuciones Linux los separan en un paquete como `python3-venv` o `python3.14-venv`; instalar el correspondiente al intérprete. La primera descarga requiere acceso a PyPI. Las dependencias se instalan en un entorno aislado. En Windows, el lanzador `preprocesador-documentos/scripts/preprocesar_documento.ps1` puede instalar Python 3.12 con `winget` para el usuario actual cuando no encuentre Python compatible; la entrada Python y el lanzador POSIX requieren un intérprete previamente instalado.

Desde la raíz de la colección de skills, preparar las dependencias PDF/Word una vez:

```sh
# Linux y macOS
python3 workflow-maestro-academico-editorial/scripts/ejecutar.py --preparar
```

```powershell
# Windows
py -3 workflow-maestro-academico-editorial/scripts/ejecutar.py --preparar
```

Ejecutar una herramienta:

```sh
python3 workflow-maestro-academico-editorial/scripts/ejecutar.py auditor-documental-academico/scripts/inventariar_documento.py "ruta/manuscrito.md"
python3 workflow-maestro-academico-editorial/scripts/ejecutar.py preprocesador-documentos/scripts/preprocesar_documento.py "ruta/documento.pdf" "ruta/salida"
```
...
El lanzador usa UTF-8, conserva los argumentos y códigos de salida y evita ejecutar mediante un shell. Sin `--preparar`, usa el entorno local si existe o el intérprete actual. Los scripts de texto no requieren instalar paquetes. El preprocesador prepara automáticamente las dependencias al recibir PDF/DOCX.
...
Los generadores de informes de la colección requieren el helper `workflow-maestro-academico-editorial/scripts/archivos_seguros.py`, incluso al ejecutarlos directamente. Validan todas las salidas declaradas antes de escribir: no pueden coincidir entre sí ni con entradas, ni ocupar directorios necesarios para otra salida. Rechazan salidas existentes salvo `--overwrite`; esta opción nunca permite reemplazar originales, perfiles, manifiestos de entrada o PDFs utilizados.
...
```text
python -m unittest discover -s workflow-maestro-academico-editorial/tests -v
...
- Mantenimiento: `workflow-maestro-academico-editorial/scripts/requirements-mantenimiento.txt` fija PyYAML para el validador oficial. Puede usarse en el intérprete de pruebas o en otro explícito.

Instalar los perfiles opcionales solo en un entorno aislado. Desde la raíz, con el Python de ese entorno:

```text
python -m pip install --only-binary=:all: -r explorador-temas-articulos/scripts/requirements-graficos-lock.txt -r gestor-imagenes-academicas-libros/scripts/requirements-lock.txt -r workflow-maestro-academico-editorial/scripts/requirements-mantenimiento.txt
python -m pip check
...
Los locks fijan versiones, no hashes de cada wheel: no garantizan identidad binaria, disponibilidad futura del índice ni resultados visuales idénticos entre sistemas. LibreOffice, Poppler, las fuentes y `render_docx.py` pertenecen al entorno de renderizado explícito, fuera de estos perfiles. Una actualización requiere cambiar el lock y repetir pruebas. Se probó instalación limpia en Linux/Python 3.12; Windows/macOS requieren validación nativa. Véase [instalaciones repetibles de pip](https://pip.pypa.io/en/stable/topics/repeatable-installs/).
...
```text
python workflow-maestro-academico-editorial/scripts/ejecutar.py workflow-maestro-academico-editorial/scripts/validar_coleccion.py --compare "ruta/a/skills-instaladas" --validator "ruta/a/skill-creator/scripts/quick_validate.py" --validator-python "ruta/al/python-con-PyYAML" --out "ruta/fuera-de-las-skills/validacion.json"
```
