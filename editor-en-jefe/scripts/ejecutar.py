...
from runtime_portable import configure_console, run_script, runtime_directory, select_python
...
def main() -> int:
    ...
    try:
        ...
        # Las herramientas que declaran su lock deben ser ejecutables desde el
        # primer uso: se prepara el runtime Python, no se deriva a LibreOffice.
        candidate_lock = script.with_name('requirements-lock.txt') if script is not None else None
        dependency_lock = candidate_lock if candidate_lock is not None and candidate_lock.is_file() else None
        requires_python_packages = dependency_lock is not None
        local_runtime = runtime_directory(script.parents[1]) if requires_python_packages else None
        python = select_python(prepare=args.preparar or requires_python_packages, lock=dependency_lock,
                               directory=local_runtime)
        if script:
