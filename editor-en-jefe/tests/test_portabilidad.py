...
RUNNER = ROOT / 'editor-en-jefe/scripts/ejecutar.py'
...
class PortabilidadTests(unittest.TestCase):
    ...
    def test_runner_executes_a_script_without_a_dependency_lock(self):
        result = self.run_script(RUNNER, 'editor-en-jefe/scripts/generar_documentacion_skills.py', '--help')
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn('Genera documentación Markdown', result.stdout)
