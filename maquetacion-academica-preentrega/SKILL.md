---
name: maquetacion-academica-preentrega
...
Para generar portada básica y encabezado, usar `--apa7-strict --apa-metadata datos.json` con el esquema de [references/apa7-docx.md](references/apa7-docx.md). Exige datos explícitos y una afiliación compartida; no combinar con plantilla. La portada generada y las notas de autor requieren revisión visual y editorial antes de entrega.

Tras cambiar la portada o la paginación del conversor, ejecutar `scripts/regresion_visual_apa.py` según [la prueba de renderizado](references/apa7-docx.md#regresión-del-renderizado). Comprueba cuatro casos controlados sobre PDF y conserva PNG para inspección; no usarlo para certificar manuscritos completos.
